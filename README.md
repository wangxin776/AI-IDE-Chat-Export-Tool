...
- **实时进度监控**：批量下载过程中提供实时进度显示、状态跟踪和取消操作
...
- **SQLite** - 数据库操作和查询
- **Flask-CORS** - 跨域资源共享支持
- **Threading** - 多线程并发处理（批量下载任务）
- **ZipFile** - ZIP文件压缩和打包功能
...
1. **进入批量模式**：在聊天列表页面点击"批量选择"按钮
2. **选择对话**：
   - 通过复选框选择要下载的对话
   - 使用"全选"按钮快速选择所有对话
   - 已选择的对话会以高亮边框显示
3. **开始批量下载**：
   - 点击"批量下载"按钮（显示已选择的对话数量）
   - 在弹出的对话框中选择导出格式（HTML、JSON 或 Markdown）
   - 点击"开始下载"确认操作
4. **监控下载进度**：
   - 实时查看下载进度条和完成状态
   - 显示已完成数量、失败数量和预计剩余时间
...
5. **下载文件**：
   - 任务完成后点击"下载文件"按钮
   - 系统会自动下载包含所有对话的ZIP压缩包
   - ZIP文件内按格式分类存储，并包含详细的元数据信息

**批量下载特性**：
- 支持最多100个对话的同时下载
- 智能错误处理：部分失败不影响其他对话的导出
- 自动文件命名：包含数据源、格式和任务ID信息
- 进度持久化：页面刷新后可继续查看任务状态
...
- **并发下载**：多线程并行处理，最多支持5个并发工作线程
- **智能打包**：自动ZIP压缩，支持大文件流式处理避免内存溢出
- **实时监控**：2秒轮询间隔，提供精确的进度跟踪和状态更新
- **容错机制**：部分失败不影响整体任务，详细记录失败原因
- **资源管理**：自动清理临时文件，支持任务取消和资源回收
- **用户体验**：友好的进度界面，支持剩余时间估算和操作引导
...
3. **批量下载系统**：全新的批量选择和下载功能，支持多线程并发处理
4. **实时进度监控**：完整的任务状态跟踪、进度显示和错误处理机制
5. **现代化界面**：全新的 Material-UI 设计和深色主题
6. **智能数据提取**：更强大的数据解析和转换能力
7. **项目识别优化**：更准确的项目名称识别算法
8. **错误处理增强**：更完善的异常处理和用户反馈
9. **设置管理系统**：可视化的配置界面和路径管理
10. **JetBrains IDE 支持**：新增 IDEA 和 PyCharm Augment 数据源支持
...
│   │   │   ├── ChatList.js        # 对话列表页面（含批量选择功能）
│   │   │   ├── BatchDownloadManager.js # 批量下载管理组件
│   │   │   ├── ProgressDialog.js  # 进度显示对话框
│   │   │   ├── Header.js          # 页面头部组件
│   │   │   ├── PathConfigCard.js  # 路径配置卡片
│   │   │   └── SettingsPage.js    # 设置页面
│   │   ├── hooks/           # 自定义Hook
│   │   │   └── useBatchDownload.js # 批量下载Hook
│   │   ├── utils/           # 工具函数
...
│   └── package.json
├── backend/                 # Python后端服务
│   ├── server.py           # Flask主服务器（含批量下载API）
│   ├── batch_download_manager.py  # 批量下载管理器
│   ├── file_packager.py    # 文件打包器
